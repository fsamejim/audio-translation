...
INPUT_FILE = "../joe-charlie-aa-js/02-japanese-translation-text/transcript_ja_04.txt" # Input dialogue text file
...
def merge_audio_chunks(output_dir=OUTPUT_DIR, result_path=MERGED_FILE, pause_ms=PAUSE_MS):
    ...
    def extract_sort_key(filename):
        ...
        match = re.match(r"(\d+)_Speaker_[A-Z]_1\.mp3", basename)
        return int(match.group(1)) if match else float('inf')

    files = sorted([
        f for f in glob.glob(os.path.join(output_dir, "*.mp3"))
        if re.search(r"\d+_Speaker_[A-Z]_1\.mp3$", f)
